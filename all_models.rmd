...
for (i in seq_along(batch_1)) {
  result <- run_model(
    Y = batch_1[[i]]$Y,
    xFormula = batch_1[[i]]$xFormula,
    maxIterOptimize = batch_1[[i]]$maxIterOptimize
  )
  fit_df[[paste0("model_", batch_1[[i]]$name, "_combo_drop_nieve")]] <- c(
    batch_1[[i]]$title,
    result$ZI_aic,
    result$ZI_ll,
    result$ZI_df,
