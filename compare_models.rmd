...
# Order top_results by the minimum of either result$ZI_aic or result$NB_aic
top_results <- top_results[order(
  sapply(top_results, function(x) {
    min(x$result$ZI_aic, x$result$NB_aic)
  })
)]


top_results <- top_results[1:15]

for (model_result in top_results) {
  model_definition <- model_result$model_definition
  result <- model_result$result
  title <- model_definitions[[i]]$title

  title_with_pc_genes <- paste(model_definitions[[i]]$title, "gcc + length", sep = " + ")
